...
for i = 1:m
    sum_k = 0;
    for j = 1:num_labels
        sum_k = sum_k - Y(y(i), j) * log(a3(i, j)) - (1 - Y(y(i), j)) * log(1 - a3(i, j));
    end
    sum_m = sum_m + sum_k;
end
...
sum_theta1 = sum_theta1 * lambda / (2 * m);
sum_theta2 = sum_theta2 * lambda / (2 * m);

J = J + sum_theta1 + sum_theta2;
